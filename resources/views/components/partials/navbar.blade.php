<header class="sticky top-0 z-50 border-b border-white/5 bg-neutral-950/80 backdrop-blur-xl">
    <nav class="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-6 py-4 lg:px-8">
        <a href="/" class="font-display text-3xl text-white tracking-tight transition hover:text-red-300 shrink-0">
            Prasunk <span class="text-red-400">Drama</span>
        </a>

        <div class="flex items-center gap-4">
            {{-- Drama / Anime Toggle --}}
            <div class="flex items-center gap-1 rounded-full border border-white/10 bg-white/[0.02] p-0.5">
                <a href="/" class="px-3 py-1.5 rounded-full text-xs font-medium transition {{ request()->is('anime*') ? 'text-neutral-400 hover:text-white' : 'bg-red-950/50 text-red-400 border border-red-900/30' }}">
                    Drama
                </a>
                <a href="{{ route('anime.index') }}" class="px-3 py-1.5 rounded-full text-xs font-medium transition {{ request()->is('anime*') ? 'bg-red-950/50 text-red-400 border border-red-900/30' : 'text-neutral-400 hover:text-white' }}">
                    Anime
                </a>
            </div>
            ...
            <details class="relative">
                <summary class="flex cursor-pointer list-none items-center gap-2 rounded-full border border-white/10 px-4 py-2 text-sm font-medium text-neutral-200 transition hover:border-red-500/30 hover:text-white">
                    @if($selectedName && isset($platformIcons[$selectedSource]))
                        <img src="{{ $platformIcons[$selectedSource] }}" alt="{{ $selectedName }}" class="h-5 w-5 rounded-sm object-cover flex-shrink-0">
                    @else
                    ...
                    @endif
                    <span>{{ $selectedName ?? 'Pilih Platform' }}</span>
