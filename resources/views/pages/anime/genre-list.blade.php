<x-layout.app :title="'Anime ' . $genreName . ' - Prasunk Anime'" :description="'Daftar anime genre ' . $genreName . ' subtitle Indonesia.'">
    ...
    <section class="mx-auto w-full max-w-7xl px-6 pt-12 pb-24 lg:px-8">
        ...
        <div class="mb-10">
            <p class="font-display text-5xl text-red-400">Kategori Anime</p>
            <h1 class="mt-2 text-3xl font-extrabold text-white flex items-center gap-3">
                <i class="ri-price-tag-3-fill text-red-500"></i> Genre <span class="text-red-500">{{ $genreName }}</span>
            </h1>
        </div>
        ...
        @if(!empty($animeList))
            ...
            <nav class="flex items-center justify-center gap-2 mt-10">
                @if($currentPage > 1)
                <a href="{{ route('anime.genre.list', ['slug' => $slug, 'page' => $prevPage]) }}" class="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 text-neutral-300 transition hover:border-red-500/30 hover:text-white hover:bg-red-500/10">
                ...
                <a href="{{ route('anime.genre.list', ['slug' => $slug, 'page' => $nextPage]) }}" class="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 text-neutral-300 transition hover:border-red-500/30 hover:text-white hover:bg-red-500/10">
        ...
        @else
            <div class="text-center py-20 border border-white/5 rounded-2xl bg-white/[0.01]">
                ...
                <p class="text-sm text-neutral-500 mt-2">Coba genre lainnya.</p>
            </div>
        @endif
    </section>
