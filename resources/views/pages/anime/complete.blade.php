<x-layout.app title="Anime Tamat - Prasunk Anime" description="Daftar anime yang sudah tamat (completed) subtitle Indonesia.">
    ...
    <section class="mx-auto w-full max-w-7xl px-6 pt-12 pb-24 lg:px-8">
        ...
        <div class="mb-8">
            <p class="font-display text-5xl text-green-400">Koleksi Lengkap</p>
            <h1 class="mt-2 text-3xl font-extrabold text-white flex items-center gap-3">
                <i class="ri-checkbox-circle-fill text-green-500"></i> Anime <span class="text-green-500">Tamat</span>
            </h1>
            <p class="text-sm text-neutral-400 mt-2">Daftar anime yang sudah tamat (completed) subtitle Indonesia.</p>
        </div>
        ...
        @if(!empty($animeList))
            ...
            <nav class="flex items-center justify-center gap-2 mt-10">
                @if($currentPage > 1)
                <a href="{{ route('anime.complete', ['page' => $prevPage]) }}" class="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 text-neutral-300 transition hover:border-green-500/30 hover:text-white hover:bg-green-500/10">
                ...
                @endif
                <span class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-green-600 text-sm font-semibold text-white shadow-lg shadow-green-600/30">{{ $currentPage }}</span>
                <a href="{{ route('anime.complete', ['page' => $nextPage]) }}" class="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 text-neutral-300 transition hover:border-green-500/30 hover:text-white hover:bg-green-500/10">
